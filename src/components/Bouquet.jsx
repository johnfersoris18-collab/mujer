export default function Bouquet({ phase }) {

  // Lógica de transición de fases:
  // Fase 1: Crece (isGrowing = true)
  // Fase 2: Descrece (isGrowing = false, quita las clases 'show' y el CSS hace el retroceso)
  // Fase 3: Desaparece el contenedor para dar paso al jardín
  const isGrowing = phase === 1;
  ...
  return(
    <div className={`absolute bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[420px] h-[600px] pointer-events-none z-10 transition-opacity duration-1000 ${isVisible ? "opacity-100" : "opacity-0"}`}>
        ...
        {branches.map(branch=>(
          ...
          >
            <path
              ...
              fill="none"
              strokeLinecap="round"
              className={`stem ${isGrowing ? "show" : ""}`}
              style={{transitionDelay:`${branch.delay}s`}}
            />

            {branch.leaves.map((leaf,i)=>(
              <g key={i} transform={`translate(${leaf.x} ${leaf.y})`}>
                <g className={`grow ${isGrowing ? "show" : ""}`} style={{transitionDelay:`${branch.delay+leaf.delay}s`}}>
            ...
            <g transform={`translate(${branch.endX} ${branch.endY})`}>
              <g className={`grow ${isGrowing ? "show" : ""}`} style={{transitionDelay:`${branch.delay+2.2}s`}}>
